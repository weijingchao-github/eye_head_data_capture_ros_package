...
class GazeEstimation:
    ...
    def do_gaze_direction_estimate(self, image_color):
        # 检测结果，根据NWU坐标系，从X轴yaw顺时针旋转为正，pitch顺时针旋转为正
        # 转化为NWU坐标系旋转正负号，需要将两个值取负号
        ...
        if self.enable_visualization:
            ...
            # cv2.waitKey(0)
            cv2.imwrite("result.jpg", image_color_plot)
