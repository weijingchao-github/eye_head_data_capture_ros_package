...
def process_json_file(input_path, output_path):
    ...
    for timestamp, entry in data.items():
        ...
        # 转换目标眼球姿态 (ZY欧拉角)
        converted_entry["target_eye_pose"] = rotation_matrix_to_zy_euler(
            entry["target_eye_pose"]
        )

        # 转换目标头部姿态 (ZYX欧拉角)
        converted_entry["target_head_pose"] = rotation_matrix_to_zyx_euler(
...
def main():
    # 创建输出文件夹
    output_dir = "euler_angle_representation"
