...
def calculate_gaze_ray_intersection(
    ...
    rectified_head_pose_quaternion_wxyz = calculate_rectified_head_pose(
        initial_head_camera_pose_quaternion_xyzw,
        current_head_camera_pose_quaternion_xyzw,
    )
    # roll, pitch, yaw = euler_from_quaternion(
    #     rectified_head_pose_quaternion_wxyz, axes="rxyz"
    # )
    rectified_head_pose_quaternion = UnitQuaternion(rectified_head_pose_quaternion_wxyz)
    eye_gaze_yaw_angle, eye_gaze_pitch_angle = eye_gaze_direction
    # eye_gaze_pitch_angle = math.radians(-3.5253)
    ...
    eye_frame_position_and_pose = rectified_head_pose_quaternion.SE3() * SE3(
        eye_origin_position_in_base_frame
    )
    # 在眼球坐标系中构建视线方向向量
    tf_gaze_ray_pose = SE3.Rz(eye_gaze_yaw_angle) * SE3.Ry(eye_gaze_pitch_angle)
